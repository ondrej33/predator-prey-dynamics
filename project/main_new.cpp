...
public:
    ...
    void step(vector<Fish_t> & visible_neighbours) {
        ...
        // be careful with neighbours over the "border" (if there is no wall)
        int N = 0;
        auto avg_p = glm::vec2(0.0f);
        for (Fish_t n : visible_neighbours) {
            // check if it is not "over the border" neighbour
            if (glm::distance(n.pos, this->pos) <= (float)shark_sense_dist) {
                avg_p += n.pos;
            } else { // if it is, handle it differently -- just go to the nearest wall point
                glm::vec2 border_point = getNearestBorderPoint<WIDTH, HEIGHT>(n.pos);
                avg_p += border_point;
            }
            N++;
        }
...
public:
    ...
    vector<Fish_t> getFishPrey(const Shark_t& s) {
        vector<Fish_t> neighbours;
        glm::vec2 nearest_wall_point = getNearestBorderPoint<WIDTH, HEIGHT>(s.pos);
        float distance_to_wall = glm::distance(nearest_wall_point, s.pos);
        ...
        for (const auto& f: swarm){
            ...
            // also check for neighbours over the "border" if there is no wall (simplified)
            float distance_over_wall = distance_to_wall + glm::distance(nearest_wall_point, f.pos);
            if (!wall && f.alive && distance_over_wall <= (float)shark_sense_dist) {
                neighbours.push_back(f);
            }
